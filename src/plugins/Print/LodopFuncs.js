...
function Message() {
    Modal.confirm({
        title: "安装CLodop打印",
        content:
            '<div>CLodop打印服务(localhost本地)未安装启动！点击这里<a href="http://demo.c-lodop.com:8000/Lodop6.226_Clodop4.142.zip">执行安装</a>安装后请刷新页面</div>',
        okText: "开始安装",
        onOk: () => {
            window.location.href = httpsUrlTrans("http://demo.c-lodop.com:8000/Lodop6.226_Clodop4.142.zip");
        },
    });
}
function upMessage() {
    Modal.confirm({
        title: "升级CLodop打印",
        content:
            '<div>CLodop打印服务(localhost本地)版本太低！点击这里<a href="http://demo.c-lodop.com:8000/Lodop6.226_Clodop4.142.zip">执行升级</a>升级后请刷新页面</div>',
        okText: "开始升级",
        onOk: () => {
            window.location.href = httpsUrlTrans("http://demo.c-lodop.com:8000/Lodop6.226_Clodop4.142.zip");
        },
...
function httpsUrlTrans(url) {
    if (url && ("https:" == document.location.protocol ? true : false) && url.indexOf("https") < 0) {
...
if (needCLodop()) {
    // var src1 = 'http://192.168.1.157:8000/CLodopfuncs.js?priority=1'
    // var src2 = 'http://192.168.1.157:8000/CLodopfuncs.js?priority=0'
    var src1 = "http://localhost:8000/CLodopfuncs.js?priority=1";
    var src2 = "http://localhost:18000/CLodopfuncs.js?priority=2";
...
function getLodop(oOBJECT, oEMBED) {
    ...
    // var strCLodopInstall1 = '<br><font color="#FF00FF">Web打印服务CLodop未安装启动，点击这里<a href="CLodop_Setup_for_Win32NT.exe" target="_self">下载执行安装</a>'
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            // =====Lodop插件未安装时提示下载地址:  || (!LODOP.VERSION)==========
            if (!LODOP) {
